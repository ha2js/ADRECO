...
def draw_gaze(image_in, eye_pos, pitchyaw, length=200, thickness=1, color=(0, 0, 255)):
    ...
    cv2.arrowedLine(image_out, tuple(np.round(eye_pos).astype(np.int32)), #화살표 시작 지점
                   tuple(np.round([eye_pos[0] + dx, eye_pos[1] + dy]).astype(int)), color, #화살표 끝 부분 좌표
                   thickness, cv2.LINE_AA, tipLength=0.5)
    d_length2 = (dx * dx ) + (dy * dy)
    d_length = math.sqrt(d_length2)
    return image_out

def zlength(pitchyaw):
    dx = -200 * np.sin(pitchyaw[1])
    dy = -200 * np.sin(pitchyaw[0])
    d_length2 = (dx * dx ) + (dy * dy)
    d_length = math.sqrt(d_length2)
    return d_length
